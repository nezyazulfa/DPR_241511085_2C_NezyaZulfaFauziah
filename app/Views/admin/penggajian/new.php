...
<?= $this->section('content') ?>
    ...
    <form action="<?= site_url('admin/penggajian') ?>" method="post">
        <?= csrf_field() ?>
        <input type="hidden" id="id_anggota_form" name="id_anggota">
...
<script>
    $(document).ready(function() {
        $('#id_anggota_select').change(function() {
            ...
            // PERBAIKAN ADA DI SINI: idAnggota (dengan 'g')
            if (idAnggota) { 
                ...
                $.ajax({
                    url: "<?= site_url('admin/penggajian/get-komponen-by-jabatan') ?>", // URL tanpa tambahan jabatan
                    type: 'POST', // Ubah GET menjadi POST
                    data: {
                        'jabatan': jabatan, // Kirim jabatan sebagai data
                        '<?= csrf_token() ?>': '<?= csrf_hash() ?>' // Penting untuk keamanan CI4
                    },
                    dataType: 'json',
                    success: function(response) {
                        ...
                        // Ganti pesan "Memuat..." dengan tabel yang sudah jadi
                        $('#komponen_gaji_list').html(html);
                    },
                    error: function() {
                         $('#komponen_gaji_list').html('<p class="text-danger">Gagal memuat komponen gaji. Periksa Rute (Routes.php) dan nama fungsi di Controller.</p>');
                    }
                });
            }
        });
